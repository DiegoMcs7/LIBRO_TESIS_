...
\justify
Para el ladrillo, los modelos GRU de ambos escenarios también difieren significativamente. El escenario sin pandemia fue optimizado con 266 pruebas de Optuna durante 465,7 minutos, convergiendo a la arquitectura más compacta de todo el estudio. El escenario con pandemia empleó la configuración predeterminada. A continuación se presenta cada escenario por separado.
...
\justify
La búsqueda de hiperparámetros con Optuna (266 ensayos, 465{,}7 min) seleccionó para el ladrillo sin pandemia una configuración marcadamente diferente: una capa GRU unidireccional con apenas 32 unidades ocultas, ventana de entrada (\textit{lookback}) de 4 meses, optimizador RMSprop con tasa de aprendizaje $\eta = 0{,}00612$ y \textit{weight decay} de $1{,}52 \times 10^{-5}$, \textit{batch size} de 32, \textit{dropout} de 0{,}05, \textit{dropout} recurrente de 0{,}1, y \textit{scheduler} CosineAnnealingLR. La arquitectura resultante totaliza únicamente 3.969 parámetros entrenables, convirtiéndose en el modelo más compacto de todo el estudio. La Tabla~\ref{tab:gru_ladrillo_sc_arq} resume los hiperparámetros.

\begin{table}[htbp]
    \centering
    \caption{Hiperparámetros del modelo GRU para ladrillo --- escenario sin pandemia (Optuna: 266 ensayos, 465{,}7 min).}
    ...
    \begin{tabular}{ll}
        ...
        Unidades ocultas       & 32 \\
        Lookback               & 4 meses \\
        Optimizador            & RMSprop ($\eta=0{,}00612$, WD $=1{,}52 \times 10^{-5}$) \\
        Batch size             & 32 \\
        Dropout / Rec.\ dropout & 0{,}05 / 0{,}1 \\
        Scheduler              & CosineAnnealingLR \\
        Parámetros entrenables & 3.969 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\justify
El modelo GRU del ladrillo para el escenario sin pandemia convergió en 97 épocas con \textit{early stopping}, requiriendo considerablemente más épocas que los demás modelos del estudio. La Figura~\ref{fig:gru_ladrillo_curva_sc} muestra las curvas de pérdida. La mayor cantidad de épocas puede atribuirse a la combinación del \textit{batch size} grande (32) con el \textit{scheduler} CosineAnnealingLR, que reduce y restaura cíclicamente la tasa de aprendizaje, permitiendo una exploración más exhaustiva del espacio de parámetros pero requiriendo más iteraciones para converger.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrr}
        ...
        \textbf{Métrica} & \textbf{Train} & \textbf{Val} & \textbf{Test} \\
        \midrule
        RMSE (Gs.) & 14{,}36 & 11{,}41 & 3{,}43 \\
        \bottomrule
    \end{tabular}
\end{table}

\justify
El RMSE de prueba de Gs.~3{,}43 es el mejor resultado individual de todo el estudio para el material ladrillo, y lo logra el modelo más compacto (3.969 parámetros). En términos relativos al rango histórico de la serie (Gs.~428--700), representa apenas el 1{,}3\,\% del rango, lo que evidencia un ajuste extraordinariamente preciso.
...
\justify
El modelo GRU proyecta, bajo el escenario sin pandemia, un precio del ladrillo notablemente estable: las predicciones oscilan entre Gs.~643 y Gs.~654 durante todo el horizonte de 24 meses, una banda excepcionalmente estrecha que refleja la naturaleza estable de este mercado. La Tabla~\ref{tab:gru_ladrillo_pred_sc} sintetiza los valores pronosticados para los primeros doce meses.

\begin{table}[htbp]
    \centering
    \caption{Pronóstico GRU del precio del ladrillo (Gs.) --- escenario sin pandemia, agosto 2025 -- julio 2026.}
    \label{tab:gru_ladrillo_pred_sc}
    \begin{tabular}{lr}
        \toprule
        \textbf{Mes} & \textbf{Precio Predicho (Gs.)} \\
        \midrule
...
\justify
Para el escenario con pandemia, la configuración seleccionada corresponde a una capa GRU unidireccional con 64 unidades ocultas, sin \textit{dropout} ni planificador. El optimizador fue Adam con $\eta = 0{,}001$ y regularización L2 $\lambda=10^{-5}$, \textit{lookback} de 6 meses y \textit{batch size} de 4. La arquitectura totaliza 14.081 parámetros entrenables, considerablemente mayor que los 3.969 del escenario sin pandemia. La Tabla~\ref{tab:gru_ladrillo_cc_arq} resume estos parámetros.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrr}
        ...
        \textbf{Métrica} & \textbf{Train} & \textbf{Val} & \textbf{Test} \\
        \midrule
        RMSE (Gs.) & 15{,}49 & 14{,}02 & 6{,}94 \\
        \bottomrule
    \end{tabular}
\end{table}

\justify
El RMSE de prueba de Gs.~6{,}94 es superior al obtenido en el escenario sin pandemia (Gs.~3{,}43), lo que refleja que el modelo más compacto y propiamente optimizado para el escenario sin pandemia supera en precisión a la configuración predeterminada del escenario con pandemia. No obstante, el error de Gs.~6{,}94 representa apenas el 2{,}6\,\% del rango histórico de la serie, resultado que sigue siendo aceptable.
...
\justify
Bajo el escenario con efecto pandémico, el modelo GRU del ladrillo proyecta una tendencia ascendente sostenida: el rango completo de precios se sitúa entre Gs.~638 y Gs.~759. Este comportamiento contrasta marcadamente con el escenario sin pandemia, donde el precio se mantenía estable en la banda de Gs.~643--654. La diferencia entre ambos escenarios del GRU para el ladrillo se amplía progresivamente a lo largo del horizonte de predicción, con el precio bajo pandemia superando significativamente al escenario base en los meses finales.

\justify
La divergencia en la interpretación del efecto pandémico entre los modelos GRU del cemento y del ladrillo constituye uno de los hallazgos más relevantes del estudio. Mientras que para el cemento la GRU asoció la pandemia con una contracción de precios (rango Gs.~46.048--53.874 con pandemia versus Gs.~55.600--61.845 sin pandemia), para el ladrillo la red aprendió a asociar la variable pandémica con un incremento sostenido de precios (Gs.~638--759 con pandemia versus Gs.~643--654 sin pandemia). Esta asimetría sugiere que el modelo GRU capturó dinámicas de mercado específicas de cada material durante el período pandémico: para el cemento, la pandemia se interpretó como un factor de contracción de la demanda; para el ladrillo, como un factor de restricción de la oferta que impulsa los precios al alza, interpretación más consistente con lo efectivamente observado durante 2020--2022.

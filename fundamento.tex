...
\begin{equation}
 ...
 \end{equation}
 donde $(\varepsilon_t)$ es ruido blanco y $(\alpha_i)$ son los coeficientes autoregresivos.
...
p: orden del componente autoregresivo (AR) no estacional, que mide la influencia de valores pasados sobre el actual.
...
\justify
El ML puede clasificarse en 3 categorias principales:
...
\justify
Las redes LSTM, introducidas por Hochreiter y Schmidhuber (1997), constituyen una variante de las RNNs desarrollada con el propósito de superar las limitaciones de estas últimas al aprender dependencias de largo plazo en secuencias.
...
\justify
Las redes GRU fueron introducidas por Cho et al. (2014) como una simplificación del modelo LSTM. El objetivo fue conservar la capacidad de las redes recurrentes para modelar dependencias a largo plazo, reduciendo al mismo tiempo la complejidad computacional y el número de parámetros.
\justify
...
\justify
La compuerta de reinicio $R_t \in \mathbb{R}^{n \times d}$ y la compuerta de actualización $Z_t \in \mathbb{R}^{n \times d}$ se calculan como:
...
\justify
donde $W_{xr}, W_{xz} \in \mathbb{R}^{d \times h}$ y $W_{hr}, W_{hz} \in \mathbb{R}^{h \times h}$ son matrices de pesos, mientras que $b_r, b_z \in \mathbb{R}^{1 \times h}$ son los términos de sesgo $bias$.
\justify
...
\item Cuando $Z_t$ se aproxima a 1, la red preserva el estado anterior, evitando que la nueva entrada lo modifique significativamente.
\end{itemize}
...
\justify
donde $W_{xh} \in \mathbb{R}^{d \times h}$ y $W_{hh} \in \mathbb{R}^{h \times h}$ son matrices de pesos, $b_h \in \mathbb{R}^{1 \times h}$ son los términos de sesgo $bias$, el símbolo $\odot$ indica el producto de Hadamard, es decir, una multiplicación elemento a elemento y y la función de activación $tanh$ limita los valores del estado candidato al intervalo (-1,1).
...
\begin{itemize}
\item Si $Z_t \approx 1$, el modelo mantiene la memoria previa $H_{t-1}$, ignorando la entrada actual.
\item Si $Z_t \approx 0$, el modelo adopta completamente el nuevo estado $\tilde{H}_t$, actualizando la información.
\end{itemize}

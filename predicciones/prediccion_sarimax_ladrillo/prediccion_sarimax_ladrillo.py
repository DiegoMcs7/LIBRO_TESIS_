# =============================================================================
# prediccion_ladrillo_sarimax.py
# =============================================================================
# MODELO SARIMAX - PREDICCIÓN DEL PRECIO DEL LADRILLO
# Descripción: Pipeline de series temporales con SARIMAX,
#              gráficos para tesis e informes en CSV.
#
# Archivos de entrada requeridos (en /kaggle/input/dataset-sarimax-ladrillo/):
#   - precios_ladrillo_interpolado.csv   : Serie histórica del precio del ladrillo
...
NOMBRE_DATASET     = "dataset-sarimax-ladrillo"

RUTA_LADRILLO       = os.path.join(RUTA_ENTRADA, NOMBRE_DATASET,
                                   "precios_ladrillo_interpolado.csv")
...
def cargar_datos_historicos(ruta: str) -> pd.DataFrame:
    """
    Carga el CSV de precios de ladrillo interpolados.
    ...
    if precio_col is None:
        for c in df.columns:
            if "precio" in c.lower():
                precio_col = c
                break
    if precio_col and precio_col != "Precio_Ladrillo":
        rename[precio_col] = "Precio_Ladrillo"
    ...
    # Conversión de tipos
    for col in ["Precio_Ladrillo", "Nivel_Rio"]:
    ...
    cols_necesarias = [c for c in ["Precio_Ladrillo", "Nivel_Rio", "Cuarentena"]
                       if c in df.columns]
...
def grafico_01_series_temporales(df: pd.DataFrame, dir_salida: str) -> str:
    ...
    fig.suptitle(
        "Series Temporales: Precio del Ladrillo, Nivel del Río e Indicador COVID-19",
        fontsize=13, fontweight="bold", y=0.99,
    )

    ax = axes[0]
    ax.plot(df.index, df["Precio_Ladrillo"], color=COLORES["train"], lw=1.8)
    ax.fill_between(df.index, df["Precio_Ladrillo"], alpha=0.08, color=COLORES["train"])
    ax.set_ylabel("Precio ($/bolsa)", fontsize=10)
    ax.set_title("Precio del Ladrillo", fontsize=11)
...
def grafico_04_correlacion(df: pd.DataFrame, dir_salida: str) -> str:
    ...
    scatter = axes[1].scatter(
        df["Nivel_Rio"], df["Precio_Ladrillo"],
    ...
    y_val = df["Precio_Ladrillo"].dropna().values
    ...
    axes[1].set_ylabel("Precio del Ladrillo", fontsize=10)
    axes[1].set_title("Precio del Ladrillo vs Nivel del Río", fontsize=11)
...
def grafico_05_patron_estacional(serie: pd.Series, dir_salida: str) -> str:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle("Patrones Estacionales del Precio del Ladrillo",
                 fontsize=13, fontweight="bold")
    ...
    axes[0].set_ylabel("Precio del Ladrillo", fontsize=10)
...
def grafico_06_pronostico_completo(
    ...
    fig.suptitle(
        "Pronóstico del Precio del Ladrillo — Modelo SARIMAX\n"
        f"RMSE={rmse_test:.2f}",
        fontsize=13, fontweight="bold",
    )
    ...
    for ax_idx, ax in enumerate(axes):
        ...
        ax.set_ylabel("Precio del Ladrillo", fontsize=10)
...
def _grafico_escenario_individual(
    ...
    ax2.set_ylabel("Precio del Ladrillo", fontsize=10)
    ax2.legend(fontsize=9)
...
def grafico_09_escenarios_covid(
    ...
    ruta_sin = _grafico_escenario_individual(
        ...
        color_linea   = "#1565C0",
        titulo        = (
            "Pronóstico del Precio del Ladrillo — Escenario Sin COVID (Cuarentena=0)\n"
    ...
    ruta_con = _grafico_escenario_individual(
        ...
        color_linea   = "#B71C1C",
        titulo        = (
            "Pronóstico del Precio del Ladrillo — Escenario Con COVID (Cuarentena=1)\n"
...
def guardar_resultados_csv(
    ...
    Guarda tres archivos CSV:
      1. pronostico_futuro_ladrillo.csv     → Pronóstico (Cuarentena=0)
    ...
    p1 = os.path.join(ruta_salida, "pronostico_futuro_ladrillo.csv")
...
def main() -> dict:
    """
    Pipeline completo de predicción SARIMAX del precio del ladrillo.

    Partición de datos: 70% Train / 15% Validación / 15% Test
    (misma partición que el modelo LSTM para comparación justa).

    Pasos:
      1. Cargar datos históricos del precio del ladrillo
    ...
    """
    print("\n" + "═" * 70)
    print("  SARIMAX — PREDICCIÓN DEL PRECIO DEL LADRILLO")
    ...
    df_ladrillo   = cargar_datos_historicos(RUTA_LADRILLO)
    ...
    print(f"{'━'*60}")
    if "Nivel_Rio" in df_ladrillo.columns:
        df_ladrillo = df_ladrillo.drop(columns=["Nivel_Rio"])
    df_hist = df_ladrillo.join(df_nivel_rio[["Nivel_Rio"]], how="left")
    ...
    endogena_Y = df_hist["Precio_Ladrillo"]
    ...
    test_estacionariedad(endogena_Y, "Precio del Ladrillo")

...
NOTA: Convertido desde prediccion_precio_cemento.py
...
CONCLUSIÓN: Para ~139 registros mensuales con 6 features y 300 trials de
...
print("=" * 70)
print("PREDICCIÓN DEL PRECIO DEL CEMENTO — GRU + OPTUNA (KAGGLE)")
print("=" * 70)
...
STUDY_NAME       = 'cemento_gru_optuna'
STORAGE_PATH     = os.path.join(ruta_salida, 'optuna_cemento_gru.db')
...
# Este .py se ejecuta con subprocess.Popen — compatible con Kaggle/papermill
# (multiprocessing.spawn NO funciona en notebooks porque __main__ no es un
# módulo importable).
...
_t0_batch = time.time()
_n0_batch = -1  # se ajusta una sola vez al iniciar

def progress_cb(study, trial):
    global _n0_batch
    n_total = len(study.trials)
    if _n0_batch < 0:
        _n0_batch = n_total - 1  # trials previos (antes de este batch)
    n_batch = n_total - _n0_batch  # trials de ESTE batch
...
# Pasar sampler y pruner explícitamente — Optuna NO los persiste en SQLite.
# Sin esto usaría TPESampler() sin seed (no reproducible) y pruner genérico.
from optuna.samplers import TPESampler
from optuna.pruners import MedianPruner
...
# Eliminar DB anterior si existe para empezar limpio
...
_python_exe = sys.executable  # mismo intérprete Python que el proceso principal
...
ax.set_ylabel('Precio (Gs.)', fontsize=12)
ax.set_title(
    f'Predicciones GRU en Todos los Splits — Precio del Cemento\n{ETIQUETA_COVID}',
    fontsize=14, fontweight='bold')
...
def predecir_iterativamente(modelo, data_sc_completa, df_futuro_rio,
    ...
    """
    Predicción recursiva (iterativa) del precio del cemento — GRU puro.
...
df_pred_futura.to_csv(
    os.path.join(ruta_salida, 'predicciones_cemento_gru.csv'),
    index=False, date_format='%Y-%m-%d')
print("\nGuardado: predicciones_cemento_gru.csv")
...
if df_reales is not None:
    ...
    if len(comparacion) > 0:
        ...
        ax.set_ylabel('Precio (Gs.)', fontsize=12)
        ax.set_title(
            f'Comparación: Predicción GRU vs Precios Reales del Cemento\n'
...
print("    predicciones_cemento_gru.csv")

...
print("=" * 70)
print("PREDICCIÓN DEL PRECIO DEL LADRILLO — LSTM + OPTUNA (KAGGLE)")
print("=" * 70)
...
ax.set_ylabel('Precio (Gs.)', fontsize=12)
ax.set_title(
    f'Predicciones LSTM en Todos los Splits — Precio del Ladrillo\n{ETIQUETA_COVID}',
    fontsize=14, fontweight='bold')
...
def predecir_iterativamente(modelo, data_sc_completa, df_futuro_rio,
    ...
    """
    Predicción recursiva (iterativa) del precio del ladrillo — LSTM puro.
...
df_pred_futura.to_csv(
    os.path.join(ruta_salida, 'predicciones_ladrillo.csv'),
    index=False, date_format='%Y-%m-%d')
print("\nGuardado: predicciones_ladrillo.csv")
...
if df_reales is not None:
    ...
    if len(comparacion) > 0:
        ...
        ax.set_ylabel('Precio (Gs.)', fontsize=12)
        ax.set_title(
            f'Comparación: Predicción LSTM vs Precios Reales del Ladrillo\n'
...
print("    predicciones_ladrillo.csv")
